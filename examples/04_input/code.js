...
function DraggableSquare(state) {
	...
	inputHandler.on('press', function(coords) {
		...
		var cx = coords[0];
		var cy = coords[1];
		if (ghost.pointInRect(cx, cy, x, y, size, size)) {
			// begin dragging
			dragging = true;
			drag_x = cx - x;
			drag_y = cy - y;
	...
	inputHandler.on('move', function(coords) {
		if (dragging) {
			...
			var max_x = 400 - size;
			var max_y = 300 - size;
			x = ghost.clamp(next_x, 0, max_x);
			y = ghost.clamp(next_y, 0, max_y);
		}
	});
...
var inputHandler = new ghost.Input(myGame);
